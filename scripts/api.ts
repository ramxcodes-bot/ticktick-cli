...
class TickTickAPI {
  ...
  // Checklist Items (managed through task items field)
  async getChecklistItems(projectId: string, taskId: string): Promise<ChecklistItem[]> {
  ...
  async createChecklistItem(
    ...
    // Get existing task and items
    const data = await this.getProjectData(projectId);
    const existingTask = data.tasks?.find((t) => t.id === taskId);
    const existingItems = existingTask?.items || [];
    ...
    // Update task with new items array
  ...
  async updateChecklistItem(
    ...
    const updatedItem = { ...existingItems[itemIndex], ...input };
    const updatedItems = [...existingItems];
    updatedItems[itemIndex] = updatedItem;

    // Update task with modified items array
    await this.request<Task>(`/task/${taskId}`, {
      method: "POST",
      body: JSON.stringify({
        id: taskId,
  ...
  async completeChecklistItem(projectId: string, taskId: string, itemId: string): Promise<ChecklistItem> {
    return this.updateChecklistItem(projectId, taskId, itemId, { status: 2 });
  }
